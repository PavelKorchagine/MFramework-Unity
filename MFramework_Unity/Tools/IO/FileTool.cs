...
namespace MFramework_Unity.Tools
{
    public class FileTool
    {
        ...
        public static void DeleteDirectory(string path)
        {
            ...
            int direLength = directorys.Length;
            //删掉所有子目录
            for (int i = 0; i < direLength; i++)
            {
        ...
        public static void CopyDirectory(string sourcePath, string destinationPath)
        {
            ...
            {
                FileSystemInfo systemInfo = fileSystemInfos[i];
                string destName = Path.Combine(destinationPath, systemInfo.Name);
                if (systemInfo is FileInfo)          //如果是文件，复制文件
                {
                    File.Copy(systemInfo.FullName, destName);
                }
                else                                    //如果是文件夹，新建文件夹，递归
                {
                    Directory.CreateDirectory(destName);
                    CopyDirectory(systemInfo.FullName, destName);
                }
            }
        }
        ...
        public static void SafeCopyDirectory(string sourcePath, string destinationPath)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    Directory.CreateDirectory(destName);
                    SafeCopyDirectory(systemInfo.FullName, destName);
                }
            }
        ...
        public static void ConvertFileEncoding(string sourceFile, string destFile, System.Text.Encoding targetEncoding)
        {
            ...
            File.WriteAllText(destFile, File.ReadAllText(sourceFile, sourEncoding), targetEncoding);
        }
        ...
        public static void RecursionFileExecute(string path, string expandName, FileExecuteHandle handle)
        {
            ...
            int allLength = allUIPrefabName.Length;

            for (int i = 0; i < allLength; i++)
            {
                var item = allUIPrefabName[i];
                try
                {
                    if (expandName == null) handle(item);
                    if (expandName != null && item.EndsWith("." + expandName)) handle(item);
                }
                catch (Exception e)
                {
                    throw new Exception("RecursionFileExecute Error :" + item + " Exception:" + e.ToString());
                }
            }
            ...
            {
                var dir = dires[i];
                RecursionFileExecute(dir, expandName, handle);
            }
        }
        #endregion
        ...
        {
            string[] allUIPrefabName = Directory.GetFiles(path);
            int length = allUIPrefabName.Length;

            for (int i = 0; i < length; i++)
            {
                string uiPrefabName = allUIPrefabName[i];
                if (ExpandFilter(uiPrefabName, expandNames))
                {
                    list.Add(uiPrefabName);
                }
            }

            string[] dires = Directory.GetDirectories(path);
            int length2 = dires.Length;

            for (int i = 0; i < length2; i++)
            {
                string dir = dires[i];
                RecursionFind(list, dir, expandNames);
            }
        }
        ...
        {
            if (expandNames == null) return true;
            if (expandNames.Length == 0) return !name.Contains(".");

            int length = expandNames.Length;
            for (int i = 0; i < length; i++) if (name.EndsWith("." + expandNames[i])) return true;
